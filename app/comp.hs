...
builtinDefs = [
            ...
            FDef "restoreVars" (Exactly 1)
              (\c [P.DynVar lname] ->
                ...
                map (\(n,v) -> P.Store
                  (P.Raw $
                    c (P.DynVar lname) ++ "(" ++ show (n :: Integer) ++ ")")
                  (P.Var v))
                (zip [1..27] ('θ':['A'..'Z']))),
            ...
            FDef "setUpEditor" (Exactly 0) (\_ [] -> "SetUpEditor "),
...
letReduce (i,b) (P.VarBind name list@(P.ListLit _) : rest) new =
...
comp (_, labels, _) (P.RawLabel s) = fromMaybe (error $ "label " ++ s ++ " not found") (lookup s labels)
...
comp _ (P.ListLit _) = "So there's bad stuff with empty lists"
...
compileString :: String -> Either Text.Parsec.Error.ParseError String
compileString str =
...
main = do
  ...
  case compileString str of
    ...
    Right v -> let fixed = map fixTheta v in
      if writeToFile settings 
        then writeFile (dir </> output settings) fixed
        else putStrLn fixed
